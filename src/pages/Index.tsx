  ...
  return (
    <AppLayout>
      <div className="p-6 lg:p-8 max-w-7xl">
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-foreground">Certificate of Insurance Tracker</h1>
          ...
          <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
            {selectedCOI && <>
                ...
                <div className="space-y-4 mt-2">
                  ...
                  {/* GL Details */}
                  {selectedCOI.glPolicy && <div className="rounded-lg border border-border p-4">
                      ...
                      <div className="space-y-2 rounded-md bg-muted/30 p-3">
                        <CoverageComplianceCheck label="Each Occurrence" value={selectedCOI.glPolicy.perOccurrenceLimit || selectedCOI.glPolicy.coverageLimit} minValue={settings?.min_gl_coverage_limit || ''} />
                        <CoverageComplianceCheck label="General Aggregate" value={selectedCOI.glPolicy.aggregateLimit || 'N/A'} minValue={settings?.min_gl_coverage_limit || ''} />
                        <div className="flex items-center justify-between text-xs">
                          ...
                          <div className="flex items-center gap-1.5">
                            ...
                            {settings?.additional_insured_required && (selectedCOI.additional_insured === 'confirmed' ? <CheckCircle2 className="h-3.5 w-3.5 text-status-valid" /> : <XCircle className="h-3.5 w-3.5 text-status-expired" />)}
                          </div>
                        </div>
                        <div className="flex items-center justify-between text-xs">
                  ...
                  {/* Umbrella / Excess */}
                  {selectedCOI.umbrellaPolicy && <div className="rounded-lg border border-border p-4">
                      ...
                      <div className="grid grid-cols-2 gap-3 text-sm">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            {(() => {const parts = (selectedCOI.umbrellaPolicy.expirationDate || '').split('/');if (parts.length !== 3) return null;const exp = new Date(+parts[2], +parts[0] - 1, +parts[1]);const days = Math.floor((exp.getTime() - Date.now()) / 86400000);return <StatusBadge status={getStatusFromDays(days)} daysUntilExpiry={days} />;})()}
                          </div>
                        </div>
                      </div>
                    </div>}
                  ...
                  {/* WC Coverage */}
                  {selectedCOI.wcPolicy ? <div className="rounded-lg border border-border p-4">
                    ...
                    </div> : settings?.wc_required ? <div className="rounded-lg border border-status-expired/30 bg-status-expired-bg p-4 flex items-center gap-2">
                  ...
                  <div className="rounded-lg border border-border p-4">
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                      {selectedCOI.coi_file_path && <DashboardFileViewButton filePath={selectedCOI.coi_file_path} label="View COI PDF" />}
                      {selectedCOI.gl_policy_file_path && <DashboardFileViewButton filePath={selectedCOI.gl_policy_file_path} label="View GL Policy PDF" />}
                      {selectedCOI.project_id && <PolicyUploadButton coiId={selectedCOI.id} projectId={selectedCOI.project_id} currentFilePath={selectedCOI.gl_policy_file_path} />}
                    </div>
                    {!selectedCOI.gl_policy_file_path && <p className="text-[11px] text-muted-foreground mt-2">Upload the GL policy PDF to enable in-depth AI review.</p>}
                  </div>

                  {/* Coverage Provisions & Policy Review (only shown after GL policy upload + AI review) */}
                  {selectedCOI.gl_policy_file_path && selectedCOI.glPolicy && selectedCOI.glPolicy.provisions.some((p) => p.status !== 'unknown') && <div className="rounded-lg border border-border p-4">
                      ...
                      <div className="space-y-2 mb-3">
                        {selectedCOI.glPolicy.provisions.map((provision) => {const Icon = provision.status === 'included' ? CheckCircle2 : provision.status === 'excluded' ? XCircle : AlertTriangle;const color = provision.status === 'included' ? 'text-status-valid' : provision.status === 'excluded' ? 'text-status-expired' : 'text-status-warning';return <div key={provision.name} className="flex items-start gap-2.5 rounded-lg bg-muted/50 px-3 py-2.5">
                      ...
                      <PolicyReviewDialog coiId={selectedCOI.id} filePath={selectedCOI.gl_policy_file_path} subcontractorName={selectedCOI.subcontractor} />
                    </div>}
                </div>
              </>}
          </DialogContent>
